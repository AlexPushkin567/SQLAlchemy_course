...
class SyncORM:
    ...
    def select_workers():
        with session_factory() as session:
            # worker_id = 1
            # workers_jask = session.get(WorkersOrm, worker_id)
    ...
    def convert_workers_to_dto():
        with session_factory() as session:
            ...
            print(f"{result_orm=}")
            result_dto = [WorkersRelDTO.model_validate(
...
class AsyncORM:
        ...
        async with async_session_factory() as session:
            query = (
                select(
                    ...
                    func.avg(ResumesOrm.compensation).cast(
                        Integer).label("avg_compensation"),
                )
                .select_from(ResumesOrm)
                .filter(and_(
    ...
    async def add_vacancies_and_replies():
        async with async_session_factory() as session:
            new_vacancy = VacanciesOrm(
                title="Python разработчик", compensation=100000)
            get_resume_1 = select(ResumesOrm).options(
                selectinload(ResumesOrm.vacancies_replied)).filter_by(id=1)
            get_resume_2 = select(ResumesOrm).options(
                selectinload(ResumesOrm.vacancies_replied)).filter_by(id=2)
